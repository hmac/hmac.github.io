...
lines or so - easily small enough to fit in a blog post.

> module Lambda where

> import Data.Maybe (fromMaybe)

The first thing we need is a representation for lambda terms.
...
Thus the Church numeral one `\f. \x. f x` becomes `Lam "f" (Lam "x" (App (Var "f") (Var "x")))`.
...
lambda expression - that is, the result when we reduce it as much as possibe.
...
one at the front. To compute the normal form we just take this first element. `Context`
contains the free variables in scope for a given expression. It's simply a map from
variable name to the expression bound to that variable.
...
For all other types of expression, `reduce` just recurses inside them and repeats.
